...
for (const tipo of ['Nfce'] as const) {
  ...
  const obter = () => fiscal.notaDaVenda(1)
  describe(`emissão ${tipo}: cota, silêncio e recuperação`, () => {
     ...
     * registro desta referência" — e como ele é o único caminho até a SEFAZ,
     * isso prova que nada foi emitido.
     */
