...
const SEED = `
  ...
  INSERT INTO produtos (id, nome, codigo_barras, preco, estoque)
    VALUES (2, 'Camiseta', '7892222222222', 50, 0);
...
const vendaDe = (variacao: number | null) => ({
  cliente_id: null,
  vendedor_id: 1,
  ...
  itens: [
    variacao == null
      ...
      : { produto_id: 2, variacao_id: variacao, quantidade: 1, preco_unitario: 50 }
  ]
})

/**
 * Dispara N vendas da mesma unidade "ao mesmo tempo", como N caixas fariam.
 *
 ...
 */
const disparar = (n: number, variacao: number | null = null) =>
  Promise.allSettled(Array.from({ length: n }, async () => criarVenda(vendaDe(variacao))))
...
describe.runIf(sqlite)('duas vendas simultâneas da última unidade', () => {
  it('estoque nunca fica negativo', () => {
    // A asserção que vale dinheiro. Estoque negativo significa que a loja
    // vendeu o que não tinha: um dos dois clientes vai embora sem a mercadoria
    // e o sistema não avisou ninguém.
    return disparar(2).then(() => {
      expect(
        estoqueProduto(1),
  ...
  it('aguenta a loja inteira brigando pela mesma unidade', async () => {
...
describe.runIf(sqlite)('a garantia por trás disso', () => {
  it('criarVenda não pode virar assíncrona', () => {
    ...
    const resultado = criarVenda(vendaDe(null))
    expect(
      resultado,
