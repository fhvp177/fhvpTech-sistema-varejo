      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Tudo que já foi vendido. Na devolução dá pra escolher item a item o que volta pro
            estoque.
          </p>
        </div>
...
const PDV: FC<{ onSair: () => void }> = ({ onSair }) => {
   ...
   *
   * ── Por que existe ──────────────────────────────────────────────────────────
   * Dois vendedores dividem o balcão e um computador só. Quem chega para atender
   ...
   * sessão (`electron/ipc/vendas.ts`). Então quem finalizar DEPOIS da troca leva
   * a venda inteira — a parte que o colega já tinha bipado junto — e a comissão
   * dela. Sem o aviso, a troca é silenciosa e o erro só aparece no fechamento do
   * mês, quando já não dá pra saber o que aconteceu no balcão.
   *
  ...
  useEffect(() => {
    ...
    // `carrinho` nas dependências (de propósito — reregistrar a cada bipe seria
