...
describe('conteúdo do cupom', () => {
  ...
  it('★ a observação da venda sai impressa, e escapada', () => {
      /*
       * É texto livre digitado pelo vendedor num HTML montado à mão. Sem escapar,
       * uma observação com "<" quebra o cupom inteiro — e o cupom quebrado só
       * aparece no papel, depois de a venda estar fechada.
       */
    const html = gerarHtmlCupomVenda(venda({ observacao: 'Troca até <15/09>' }), LOJA)
    expect(html).toContain('OBS.:')
    expect(html).toContain('Troca até &lt;15/09&gt;')
