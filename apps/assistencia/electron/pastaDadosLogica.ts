...
// Nome oficial da pasta de dados (%APPDATA%\<nome>). Cada nicho tem a sua e
// elas NUNCA se cruzam.
export const PASTA_OFICIAL = 'FHVP Tech Assistencia'

// A assistência nasceu já com o nome definitivo: nunca gravou em outra pasta,
// então não há nome legado nem migração a fazer. A lista existe (vazia) porque
// o mecanismo do varejo fica de pé pra um eventual rename futuro — e pra que
// portar um fix de lá siga sendo copiar-colar.
export const PASTAS_LEGADAS: string[] = []

// ⚠️ SEGURANÇA — por que esta lista NÃO entra no resolvedor de pasta.
// São as pastas de OUTROS apps FHVP na mesma máquina. Se elas fossem tratadas
// como candidatas, um PC com varejo e assistência instalados abriria o banco do
// vizinho — o app erraria de loja inteira. Elas só servem pra RECONHECER um
// caminho de backup herdado (ver pareceBackupsDeInstalacao), nunca pra eleger
// onde os dados moram.
//
// Precisam ser reconhecidas porque restaurar aqui o backup de um cliente do
// varejo é fluxo previsto: o banco restaurado traz `backup_pasta_padrao`
// apontando pra pasta DELE, e sem esta correção a assistência passaria a
// escrever backup dentro da pasta do varejo.
const PASTAS_DE_OUTROS_APPS = ['FHVP Tech Varejo', 'Sistema RT', 'sistema-rt']
...
export function corrigirCaminhosBackup(
  ...
  if (secundaria) {
    ...
    for (const legado of [...PASTAS_LEGADAS, ...PASTAS_DE_OUTROS_APPS]) {
...
function pareceBackupsDeInstalacao(caminho: string): boolean {
  ...
  return [PASTA_OFICIAL, ...PASTAS_LEGADAS, ...PASTAS_DE_OUTROS_APPS].some((nome) =>
