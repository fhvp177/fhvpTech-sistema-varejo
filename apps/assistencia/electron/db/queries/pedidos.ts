...
function moverReserva(
  ...
  for (const item of itens) {
    if (item.variacao_id != null) {
      ...
      /*
       * ⚠️ `tipo != 'servico'` no próprio UPDATE, como no decremento de
       * estoque da venda.
       *
       * Aqui peça e serviço dividem a tabela `produtos`, e código vindo do
       * varejo não sabe disso. Mão de obra não tem estoque para reservar: sem
       * esta guarda, um pedido com serviço deixaria `reservado` positivo para
       * sempre num item cujo estoque é zero — e aí `estoque - reservado` fica
       * NEGATIVO e o serviço some do balcão sem explicação.
       *
       * A trava vai no UPDATE, e não só em quem chama, porque quem chama muda.
       */
      db.prepare(
        "UPDATE produtos SET reservado = MAX(0, reservado + ?) WHERE id = ? AND tipo != 'servico'"
      ).run(sinal * item.quantidade, item.produto_id)
    }
  }
}
  ...
  return db.transaction(() => {
    ...
    for (const item of dados.itens) {
      if (item.variacao_id != null) {
        ...
        const p = db
          .prepare(
            'SELECT nome, tipo, estoque - reservado AS disponivel FROM produtos WHERE id = ?'
          )
          .get(item.produto_id) as
          | { nome: string; tipo: string; disponivel: number }
          | undefined
        if (!p) throw new Error(`Produto #${item.produto_id} não encontrado.`)
        // Serviço não tem estoque — a quantidade é livre, igual na venda.
        if (p.tipo !== 'servico' && item.quantidade > p.disponivel) {
          throw new Error(
