  ...
  const itens = db
    .prepare(
      `SELECT p.nome || CASE WHEN pv.tamanho IS NOT NULL THEN ' (' || pv.tamanho || ')' ELSE '' END AS nome,
       ...
       WHERE iv.venda_id = ?
         -- ⚠️ SERVIÇO NÃO ENTRA NA NFC-e/NF-e. Mercadoria é tributada pelo
         -- estado; serviço, pelo município — são documentos diferentes, e o
         -- serviço sai na NFS-e (vendaParaNotaServico, abaixo). Sem este filtro
         -- a venda mista de uma assistência (peça + mão de obra) travava na
         -- montagem com "o produto X está sem NCM", que é verdade e não tem
         -- conserto: serviço não tem NCM nem deve ter.
         AND p.tipo != 'servico'
       ORDER BY iv.id`
    )
...
// ─── NFS-e: o lado do serviço ─────────────────────────────────────────────────

export type ItemServicoFiscal = {
  nome: string
...
export type VendaServicoFiscal = {
  id: number
  total: number
  desconto: number
  cancelada: number
  tomador: DestinatarioFiscal | null
  servicos: ItemServicoFiscal[]
}

/**
 * A mesma venda, vista pelo lado do serviço.
 *
 * Devolve `servicos` vazio quando a venda só teve mercadoria — quem chama usa
 * isso pra decidir se o botão de NFS-e sequer aparece.
 */
export function vendaParaNotaServico(vendaId: number): VendaServicoFiscal | null {
  const db = obterBancoDeDados()

  const linha = db
    .prepare(
      `SELECT v.id, v.total, v.desconto, v.cancelada,
              c.nome AS cliente_nome, c.cpf AS cliente_cpf, c.cnpj AS cliente_cnpj,
    ...
    .get(vendaId) as Record<string, string | number | null> | undefined
  if (!linha) return null

  // Na NFS-e o tomador é sempre identificado — não existe "nota de serviço ao
  // consumidor anônimo" como a NFC-e permite.
  const tomador: DestinatarioFiscal | null = linha.cliente_nome
    ? {
        nome: (linha.razao_social as string) || (linha.cliente_nome as string),
        cnpj: linha.cliente_cnpj as string | null,
        cpf: linha.cliente_cpf as string | null,
        logradouro: linha.endereco_logradouro as string | null,
        numero: linha.endereco_numero as string | null,
        complemento: linha.endereco_complemento as string | null,
        bairro: linha.endereco_bairro as string | null,
        cidade: linha.cidade as string | null,
        uf: linha.uf as string | null,
        cep: linha.cep as string | null,
        codigo_municipio: linha.codigo_municipio as string | null,
  ...
  const servicos = db
    .prepare(
      `SELECT p.nome,
              p.item_lista_servico, p.aliquota_iss,
              p.codigo_tributacao_municipio, p.codigo_cnae,
              iv.quantidade, iv.preco_unitario AS valor_unitario
       FROM itens_venda iv
       JOIN produtos p ON p.id = iv.produto_id
       WHERE iv.venda_id = ? AND p.tipo = 'servico'
       ORDER BY iv.id`
    )
    .all(vendaId) as ItemServicoFiscal[]

  return {
    id: linha.id as number,
    total: linha.total as number,
    desconto: (linha.desconto as number) ?? 0,
    cancelada: (linha.cancelada as number) ?? 0,
    tomador,
    servicos
  }
}

// ─── Registro local das NFS-e emitidas ────────────────────────────────────────
// Espelha `notaDaVenda` e companhia, mas na tabela `nfse_emitidas` — a razão de
// serem duas tabelas está na migration at_001. A autoridade sobre numeração e
// idempotência continua sendo o backend.

export type NotaServicoDaVenda = {
  id: number
  venda_id: number
  tentativa: number
  referencia: string
...
export function notaServicoDaVenda(vendaId: number): NotaServicoDaVenda | null {
  const db = obterBancoDeDados()
  return (
    (db
      .prepare(
        `SELECT * FROM nfse_emitidas WHERE venda_id = ? ORDER BY tentativa DESC LIMIT 1`
      )
      .get(vendaId) as NotaServicoDaVenda | undefined) ?? null
  )
}

export function notasServicoDasVendas(vendaIds: number[]): Record<number, NotaServicoDaVenda> {
  if (!vendaIds.length) return {}
  const db = obterBancoDeDados()
  const marcadores = vendaIds.map(() => '?').join(',')
  const linhas = db
    .prepare(
      `SELECT n.* FROM nfse_emitidas n
       JOIN (
         SELECT venda_id, MAX(tentativa) AS t FROM nfse_emitidas
         WHERE venda_id IN (${marcadores}) GROUP BY venda_id
       ) u ON u.venda_id = n.venda_id AND u.t = n.tentativa`
    )
    .all(...vendaIds) as NotaServicoDaVenda[]
  return Object.fromEntries(linhas.map((n) => [n.venda_id, n]))
}

export function proximaTentativaServico(vendaId: number): number {
  const db = obterBancoDeDados()
  const r = db
    .prepare('SELECT COALESCE(MAX(tentativa), 0) AS t FROM nfse_emitidas WHERE venda_id = ?')
    .get(vendaId) as { t: number }
  return r.t + 1
}

export function registrarNotaServicoLocal(dados: {
  venda_id: number
  tentativa: number
  referencia: string
  acbr_id: string | null
...
export function atualizarStatusNotaServicoLocal(
  referencia: string,
  status: string,
  ...
  db.prepare(
    `UPDATE nfse_emitidas
      ...
      WHERE referencia = ?`
  ).run(status, codigoVerificacao, motivo, numero ?? null, referencia)
}

export type FiscalServico = {
...
export function obterFiscalServico(id: number): FiscalServico | null {
  const db = obterBancoDeDados()
  const r = db
    .prepare(
      `SELECT item_lista_servico, aliquota_iss, codigo_tributacao_municipio, codigo_cnae
       FROM produtos WHERE id = ? AND tipo = 'servico'`
    )
    .get(id) as Record<string, string | number | null> | undefined
  if (!r) return null
  return {
    item_lista_servico: (r.item_lista_servico as string) ?? '',
    // Vira texto pra tela; número só na hora de emitir.
    aliquota_iss: r.aliquota_iss == null ? '' : String(r.aliquota_iss),
    codigo_tributacao_municipio: (r.codigo_tributacao_municipio as string) ?? '',
    codigo_cnae: (r.codigo_cnae as string) ?? ''
  }
}

export function salvarFiscalServico(id: number, dados: FiscalServico): void {
  const db = obterBancoDeDados()
  const aliquota = dados.aliquota_iss.trim().replace(',', '.')
  const numero = aliquota === '' ? null : Number(aliquota)
  if (numero !== null && (!Number.isFinite(numero) || numero < 0 || numero > 100)) {
    throw new Error('A alíquota de ISS vai em percentual, entre 0 e 100 (ex.: 5 para 5%).')
  }
  db.prepare(
    `UPDATE produtos
        SET item_lista_servico = ?, aliquota_iss = ?,
            codigo_tributacao_municipio = ?, codigo_cnae = ?
      WHERE id = ? AND tipo = 'servico'`
  ).run(
    dados.item_lista_servico.trim() || null,
    numero,
    dados.codigo_tributacao_municipio.trim() || null,
    dados.codigo_cnae.trim() || null,
    id
  )
}

export type ServicoClassificacao = {
  id: number
  nome: string
  preco: number
  item_lista_servico: string | null
...
/** Todos os serviços do catálogo, com a classificação que já tenham. */
export function listarServicosParaClassificar(): ServicoClassificacao[] {
  const db = obterBancoDeDados()
  return db
    .prepare(
      `SELECT id, nome, preco, item_lista_servico, aliquota_iss
       FROM produtos WHERE tipo = 'servico'
       ORDER BY item_lista_servico IS NOT NULL, nome COLLATE NOCASE`
    )
    .all() as ServicoClassificacao[]
}

/**
 * Quantos serviços ainda não dá pra pôr em nota.
 *
 * O par item da LC 116 + alíquota é indivisível: com um só, a nota é recusada
 * pela prefeitura. Por isso "pendente" é ter QUALQUER um dos dois em branco, e
 * não os dois.
 */
export function diagnosticoNfse(limiteExemplos = 20): {
  total_servicos: number
  servicos_pendentes: number
  exemplos: Array<{ id: number; nome: string }>
} {
  const db = obterBancoDeDados()
  const cond = `tipo = 'servico' AND (item_lista_servico IS NULL OR TRIM(item_lista_servico) = ''
                 OR aliquota_iss IS NULL)`
  const total = (
    db.prepare(`SELECT COUNT(*) c FROM produtos WHERE tipo = 'servico'`).get() as { c: number }
  ).c
  const pendentes = (
    db.prepare(`SELECT COUNT(*) c FROM produtos WHERE ${cond}`).get() as { c: number }
  ).c
  const exemplos = db
    .prepare(`SELECT id, nome FROM produtos WHERE ${cond} ORDER BY nome COLLATE NOCASE LIMIT ?`)
    .all(limiteExemplos) as Array<{ id: number; nome: string }>
  return { total_servicos: total, servicos_pendentes: pendentes, exemplos }
}
...
export function diagnosticoFiscal(limiteExemplos = 20): DiagnosticoFiscal {
  ...
  // ⚠️ SERVIÇO FICA DE FORA DE TODA ESTA CONTA. Ele não tem NCM e nunca vai
  // ter — NCM classifica mercadoria. Contando serviço, o painel diria pra sempre
  // "faltam N produtos sem NCM", a loja nunca chegaria em "tudo pronto" e o
  // lojista iria atrás do contador pedindo um número que não existe. A
  // pendência de serviço é outra e tem contador próprio (diagnosticoNfse).
  const total = db
    .prepare(`SELECT COUNT(*) AS n FROM produtos WHERE tipo != 'servico'`)
    .get() as { n: number }

  const semNcm = db
    .prepare(
      `SELECT COUNT(*) AS n FROM produtos
       WHERE tipo != 'servico' AND (ncm IS NULL OR TRIM(ncm) = '')`
    )
    .get() as { n: number }
