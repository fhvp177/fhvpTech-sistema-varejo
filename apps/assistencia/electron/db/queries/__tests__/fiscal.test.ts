...
const SCHEMA = `
  CREATE TABLE produtos (
    ...
    -- Coluna própria da assistência (migration 027): peça e serviço moram na
    -- mesma tabela. É ela que separa o que sai na NFC-e do que sai na NFS-e.
    tipo TEXT NOT NULL DEFAULT 'produto',
    ncm TEXT, cfop TEXT, cst_csosn TEXT, origem TEXT DEFAULT '0', unidade TEXT DEFAULT 'UN',
    item_lista_servico TEXT, aliquota_iss REAL,
    codigo_tributacao_municipio TEXT, codigo_cnae TEXT
  );
...
beforeAll(() => {
  ...
  db.exec(`
    ...
    -- Um serviço, pra provar que ele NÃO entra na nota de mercadoria.
    INSERT INTO produtos (id, nome, tipo, preco, item_lista_servico, aliquota_iss)
      VALUES (3, 'Formatacao com backup', 'servico', 120, '14.02', 5);
    ...
    INSERT INTO itens_venda (venda_id, produto_id, variacao_id, quantidade, preco_unitario)
      VALUES (1, 1, 1, 2, 39.9);
    -- venda 90: MISTA (peça + mão de obra) — o caso normal de uma assistência.
    INSERT INTO vendas (id, cliente_id, total, desconto) VALUES (90, 1, 159.9, 0);
    INSERT INTO itens_venda (venda_id, produto_id, quantidade, preco_unitario)
      VALUES (90, 1, 1, 39.9), (90, 3, 1, 120);
...
describe.runIf(sqlite)('forma de pagamento e diagnóstico', () => {
  ...
  // Serviço não tem NCM e nunca vai ter. Se ele entrasse nesta conta, o painel
  // diria pra sempre "faltam produtos sem NCM" e a loja jamais ficaria "pronta".
  it('diagnóstico de NCM IGNORA serviço', () => {
    const d = diagnosticoFiscal()
    expect(d.exemplos_sem_ncm.map((e) => e.nome)).not.toContain('Formatacao com backup')
  })

  it('diagnóstico da NFS-e conta só os serviços, com pendência própria', () => {
    const d = diagnosticoNfse()
    expect(d.total_servicos).toBe(1)
    // O serviço do fixture já tem item da LC 116 e alíquota.
    expect(d.servicos_pendentes).toBe(0)
  })
})

// ── A separação que a assistência exige: mercadoria e serviço em notas
// diferentes. Mercadoria é tributada pelo estado, serviço pelo município.
describe('venda mista: cada item vai pra sua nota', () => {
  it('a nota de MERCADORIA não leva o serviço', () => {
    const v = vendaParaNota(90)
    const nomes = v!.itens.map((i) => i.nome)
    expect(nomes).toContain('Camiseta')
    expect(nomes).not.toContain('Formatacao com backup')
  })

  it('a nota de SERVIÇO não leva a mercadoria', () => {
    const v = vendaParaNotaServico(90)
    const nomes = v!.servicos.map((s) => s.nome)
    expect(nomes).toEqual(['Formatacao com backup'])
  })

  it('a nota de serviço traz o item da LC 116 e a alíquota de ISS', () => {
    const s = vendaParaNotaServico(90)!.servicos[0]
    expect(s.item_lista_servico).toBe('14.02')
    expect(s.aliquota_iss).toBe(5)
  })

  it('venda só de mercadoria devolve lista de serviços vazia', () => {
    const v = vendaParaNotaServico(1)
    expect(v!.servicos).toEqual([])
  })
})
