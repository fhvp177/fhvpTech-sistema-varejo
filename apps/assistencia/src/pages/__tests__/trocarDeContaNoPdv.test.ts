...
describe('o aviso enxerga o carrinho de agora, não o de quando a tela abriu', () => {
  ...
  it('e o ref é reapontado a cada render, que é o que o mantém fresco', () => {
    ...
    // A intenção é "este efeito NÃO tem array de dependências". A primeira
    // versão comparava os 6 caracteres seguintes com uma string literal, e isso
    // dependia da quebra de linha do arquivo: passava no varejo (LF) e falhava
    // na assistência (CRLF), sem haver problema nenhum no código.
    const janela = VENDAS.slice(i, i + 200)
    expect(janela, 'o efeito não fecha onde se esperava').toContain('})')
    expect(
      janela,
