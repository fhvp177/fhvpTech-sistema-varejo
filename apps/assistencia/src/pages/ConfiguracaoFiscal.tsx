...
const ConfigNfse = lazy(() => import('@/components/ConfigNfse'))
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        O que você <strong>vende</strong> e o que você <strong>faz</strong> saem em notas
        diferentes, porque quem cobra imposto é diferente. Peça vendida vai para a{' '}
        <strong>NFC-e</strong>, na mesma impressora térmica dos seus cupons, ou para a{' '}
        <strong>NF-e</strong> em folha A4 quando o cliente é empresa. Já a mão de obra —
        conserto, instalação, manutenção — vai para a <strong>NFS-e</strong>, que é da
        prefeitura. Numa ordem de serviço com peça e serviço juntos, saem as duas.
      </p>
      ...
      <div className="space-y-4">
        ...
        <Passo
          numero={5}
          titulo="Nota de serviço (NFS-e)"
          descricao="A nota do conserto e da instalação — essa vai pra prefeitura, não pra SEFAZ."
          estado={
            config.nfse_cidade_atendida === 'nao'
              ? 'bloqueado'
        ...
        >
          <Suspense fallback={null}>
            <ConfigNfse
              config={config}
              passo1Feito={passo1Feito}
              empresaPronta={config.empresa_cadastrada && certificadoOk}
              onSalvar={async (parcial) => {
                const atualizada = { ...config, ...parcial }
                setConfig(atualizada)
                const r = await window.api.fiscal.salvar(atualizada)
                if (!r.success) {
                  setFeedback({ tipo: 'erro', msg: r.error })
                  return false
                }
