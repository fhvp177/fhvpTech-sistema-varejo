...
import { FileDown, Printer, FolderDown, ShoppingCart, Package, BookOpen, FileText, Receipt, BarChart3, Landmark, Scale, PackageCheck } from 'lucide-react'
...
const Relatorios: FC = () => {
  ...
  // O balanço de estoque só conta mercadoria física; a tabela de referências
  // inclui serviço, que também é chamado pela referência curta no PDV.
  ...
  const gerarEstoque = async (acao: Acao) => {
    ...
    try {
      const erro = await entregar(
        gerarHtmlRelatorioEstoque(produtosFisicos),
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <CardRelatorio
          ...
          descricao="Peças por categoria com estoque do sistema e coluna em branco pra contagem física. Serviço não entra — não há o que contar."
        >
          <span className="text-xs text-muted-foreground">{produtosFisicos.length} peça(s)</span>
          <BotoesGerar
            onGerar={gerarEstoque}
            desabilitado={produtosFisicos.length === 0}
            gerando={gerando}
          />
        </CardRelatorio>
        ...
        <CardRelatorio
          ...
          descricao="Só referência + nome, em colunas compactas — a cola pra deixar no balcão. Inclui os serviços."
        >
          <span className="text-xs text-muted-foreground">{produtos.length} item(ns)</span>
          <BotoesGerar
        ...
        */}
        ...
        <CardRelatorio
          ...
          descricao="O que está apartado agora, para quem, e há quantos dias. Peça parada que ainda não virou venda."
        >
