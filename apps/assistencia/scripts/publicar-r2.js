...
// A assistência tem canal PRÓPRIO dentro do mesmo bucket. Sem este prefixo os
// dois apps escreveriam o mesmo latest.yml e uma loja de varejo receberia o
// instalador da assistência (e vice-versa) pelo auto-update.
const CANAL = `assistencia/${edicao}`
...
async function principal() {
  ...
  await put(`${CANAL}/${nomeRemoto}`, exe, 'application/octet-stream', 'public, max-age=31536000, immutable')
  console.log(`[publicar-r2:${edicao}] subindo blockmap...`)
  await put(`${CANAL}/${nomeRemoto}.blockmap`, blockmap, 'application/octet-stream', 'public, max-age=31536000, immutable')
  console.log(`[publicar-r2:${edicao}] subindo latest.yml (v${versao})...`)
  await put(`${CANAL}/latest.yml`, Buffer.from(latestYml), 'text/yaml', 'no-cache')
  console.log(`[publicar-r2:${edicao}] canal atualizado: https://updates.fhvptech.com/${CANAL}/latest.yml`)
}
