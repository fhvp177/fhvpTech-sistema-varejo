...
type EdicaoState = { nome: string; email: string }

const sanitizarPin = (v: string) => v.replace(/\D/g, '').slice(0, 6)

const CadastroUsuarios: FC = () => {
  const { sessao: logado } = useSessao()
  ...
  const carregar = async () => {
    const resp = await window.api.usuarios.listar()
    if (resp.success) setUsuarios(resp.data as Usuario[])
  }
  ...
  const criar = async () => {
    ...
    const resp = await window.api.usuarios.criar({
      nome: novoNome.trim(),
  ...
  const salvarEdicao = async (id: number) => {
    ...
    const resp = await window.api.usuarios.atualizar(id, {
  ...
  const alternarAtivo = async (u: Usuario) => {
    setErro('')
    const resp = await window.api.usuarios.alternarAtivo(u.id, u.ativo === 0)
    if (!resp.success) {
      setErro(resp.error)
      return
    }
  ...
  const alterarPapel = async (u: Usuario) => {
    setErro('')
    const novoPapel: 'dono' | 'funcionario' = u.papel === 'dono' ? 'funcionario' : 'dono'
    const confirmacao =
      novoPapel === 'dono'
        ? `Promover "${u.nome}" a Dono? Ele terá acesso total ao sistema.`
        : `Rebaixar "${u.nome}" a Funcionário? Ele perderá acesso às áreas restritas.`
    if (!confirm(confirmacao)) return
    const resp = await window.api.usuarios.alterarPapel(u.id, novoPapel)
    if (!resp.success) {
      setErro(resp.error)
      return
    }
  ...
  const excluir = async (u: Usuario) => {
    ...
    const resp = await window.api.usuarios.deletar(u.id)
    if (!resp.success) {
      setErro(resp.error)
      return
    }
  ...
  const abrirRedefinirPin = (u: Usuario) => {
    setPinAlvo(u)
    setPinNovo('')
    setPinConfirmacao('')
    setErroPin('')
  ...
  const confirmarRedefinirPin = async () => {
    ...
    setSalvandoPin(true)
    const resp = await window.api.usuarios.redefinirPin(pinAlvo.id, pinNovo)
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          ...
          <Input
            ...
            className="flex-1"
          />
          <Button onClick={criar} size="icon" title="Adicionar usuário">
            <Plus className="w-4 h-4" />
          </Button>
        </div>
        <p className="text-xs text-muted-foreground">
          Usuários novos entram como "Funcionário" e definem o próprio PIN no 1º acesso.
        </p>
      </div>
      ...
      <div className="border rounded-lg max-h-96 overflow-y-auto">
        {usuarios.length === 0 ? (
          ...
          <ul className="divide-y">
            {usuarios.map((u) => {
              const ehDono = u.papel === 'dono'
              const ehSessaoAtual = logado?.id === u.id
              ...
              return (
                <li
                  key={u.id}
                  className={`flex items-center gap-2 px-3 py-2.5 ${u.ativo === 0 ? 'opacity-60' : ''}`}
                >
                  <div
                  ...
                  <div className="flex items-center gap-0.5">
                    ...
                    <button
                      ...
                      title={u.ativo === 1 ? 'Desativar (some do login)' : 'Reativar'}
                    >
      ...
      <Dialog open={pinModalAberto} onOpenChange={setPinModalAberto}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-1">
            <p className="text-xs text-muted-foreground">
              Define um novo PIN para este usuário. Útil quando ele esquece o PIN. Avise o PIN
