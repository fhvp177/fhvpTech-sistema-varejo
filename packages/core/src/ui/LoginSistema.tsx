...
import ModalRecuperacaoPin from './ModalRecuperacaoPin'

type PapelUsuario = 'dono' | 'funcionario'
...
type Props = {
  onDesbloquear: () => void
  // Rótulos de papel exibidos na lista/seleção. Cada nicho pode ajustar
  // ("Dono da loja"/"Vendedor" no varejo; "Dono"/"Funcionário" na vet).
  rotuloDono?: string
  rotuloFuncionario?: string
}

const MAX_TENTATIVAS = 5
const SEGUNDOS_BLOQUEIO = 30

const LoginSistema: FC<Props> = ({
  onDesbloquear,
  rotuloDono = 'Dono',
  rotuloFuncionario = 'Funcionário'
}) => {
  ...
  // Auto-seleciona se houver apenas um usuário — ninguém precisa "clicar em si mesmo".
  useEffect(() => {
    if (!carregandoLista && usuarios.length === 1 && !selecionado) {
      setSelecionado(usuarios[0])
    }
  }, [carregandoLista, usuarios, selecionado])
  ...
  const carregarUsuarios = async () => {
    ...
    setErroLista('')
    const resp = await window.api.auth.listarUsuariosParaLogin()
  ...
  const rotuloPapel = (papel: PapelUsuario) => (papel === 'dono' ? rotuloDono : rotuloFuncionario)
    ...
    if (erroLista) {
      return (
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">
          <p className="font-semibold mb-1">Não foi possível carregar os usuários</p>
          <p>{erroLista}</p>
        </div>
      )
    }
    ...
    if (usuarios.length === 0) {
      return (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
          Nenhum usuário cadastrado. Entre em contato com o suporte.
        </div>
      )
    }
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm ${
                      u.papel === 'dono' ? 'bg-amber-500' : 'bg-slate-500'
                    }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 flex items-center gap-1">
                      {u.papel === 'dono' ? (
                        <>
                          <Crown className="w-3 h-3" /> {rotuloDono}
                        </>
                      ) : (
                        <>
                          <UserIcon className="w-3 h-3" /> {rotuloFuncionario}
                        </>
                      )}
                      {u.tem_pin === 0 && (
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3 bg-slate-50 rounded-lg p-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-500">{rotuloPapel(selecionado.papel)}</p>
          </div>
          {usuarios.length > 1 && (
            <button
        ...
        <div>
          ...
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === 'Escape' && usuarios.length > 1) voltarParaLista()
            }}
